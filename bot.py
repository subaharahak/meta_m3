...
✗ Creating text file with valid cards""", reply_to_message_id=msg.message_id)
    ...
    def generate_and_send_file():
        try:
            # Generate cards (more for text file - 50 cards)
...
✗ Contact admin if you need help: @mhitzxg""")
            ...
            file_content = f"Generated Cards - BIN: {bin_code}\n"
            file_content += f"Amount: {len(cards)}\n"
            file_content += "Format: CC|MM|YY|CVV\n"
            file_content += "=" * 50 + "\n\n"
            
            for i, card in enumerate(cards, 1):
                file_content += f"{card}\n"
            
            file_content += f"\nGenerated by: {msg.from_user.first_name}"
            file_content += f"\nPowered by @mhitzxg & @pr0xy_xd"
